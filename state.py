...
FOOD_ENTRIES = Constants.FOOD_ENTRIES
FOOD = Constants.FOOD
FOOD_TABLE = FAMILY_NAME + TABLES + FOOD[:58]

def getBatchAddress(batchID):
    return TRACKING_TABLE + hash(batchID)[:58]

def getFoodAddress(foodID):
    return TRACKING_TABLE + hash(foodID)[:58]
...
def getFoodAddress(foodName):
    return FAMILY_NAME + FOOD_ENTRIES + hash(foodName)[:58]
...
class PharmaState():
    ...
    def _manufacture(self, context, manufacturerName, medicineName, batchID, manufactureDate, expiryDate):
        ...
        #l = [manufacturerName, medicineName, batchID, manufactureDate, expiryDate, owner]
    @classmethod
    def _manufactureFood(self, context, foodID,data):
        foodAddress = getFoodAddress(foodID)
        try:
            LOGGER.info("entering food entry")
            foods = self._readData(context, FOOD_TABLE)
            LOGGER.info ('Foods: {}'.format(food))
            if foods:
                if foods:
                    foods.append(foodID)
                    tracking = [data]
                    ...
                    addresses = context.set_state({
                        FOOD_TABLE: self._encode_data(foodID),
                        foodAddress: self._encode_data(tracking)
                    })
                else:
                    raise Exception('no food table ')
            else:
                raise Exception('no food table')
        except Exception as e:
            logging.debug ('excecption: {}'.format(e))
